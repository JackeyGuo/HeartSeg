...
from core.layers import _conv2d, _active_layer, deconv2d, conv_bn_prelu, deconv_bn_prelu, atrous_bn_prelu, deform_conv2d, BilinearUpsample2d
...
class deform_xception_aspp(object):
    ...
    def feature_pyramid(self,_input,block_num):
        ...
        for i in range(1, self.atrou_num + 1):
            ...
            output = deform_conv2d(_input,output_channels=out_feature,kernel_size=(3,3),name='deform_conv%d'%i,norm=True)
    ...
    def inference_op(self, _input):
        ...
        # forth level
        auxiliary3_prob_4x = _conv2d(inputs=dconv_layer[0], output_feature=self.output_labels, kernel_size=1,
                                     stride=1, use_bias=True, name='auxiliary3_prob_4x')
        auxiliary3_prob_2x = deconv2d(inputs=auxiliary3_prob_4x, output_channels=self.output_labels,
                                      name='auxiliary3_prob_2x')
        auxiliary3_prob_1x = deconv2d(inputs=auxiliary3_prob_2x, output_channels=self.output_labels,
                                      name='auxiliary3_prob_1x')
        # third level
        auxiliary2_prob_2x = _conv2d(inputs=dconv_layer[1], output_feature=self.output_labels, kernel_size=1,
                                     stride=1, use_bias=True, name='auxiliary2_prob_2x')
        auxiliary2_prob_1x = deconv2d(inputs=auxiliary2_prob_2x, output_channels=self.output_labels,
                                      name='auxiliary2_prob_2x')
        # second level
        auxiliary1_prob_1x = _conv2d(inputs=dconv_layer[2], output_feature=self.output_labels, kernel_size=1,
        ...
        with tf.variable_scope('last_stage'):
            ...
            output = _conv2d(output3, kernel_size=1, stride=1, output_feature=self.output_labels, use_bias=True, name='fc_layer')
