...
class model(object):
    def __init__(self, is_training):
        ...
        self.output_channels = {'1': [32, 32, 32], '2': [64, 64, 64], '3': [128, 128, 128], '4': [64, 64], '5': [64, 32],'6': [32, 16]}
    ...
    def feature_pyramid(self,_input,block_num):
        ...
        output_1x1 = _conv2d(_input, kernel_size=1, stride=1, output_feature=out_feature, use_bias=False, name='pyramid_conv_1')
        ...
        for i in range(1, self.atrou_num + 1):
            ...
            output = atrous_bn_prelu(_input, kernel_size=3, stride=1, output_channels=out_feature,
        ...
        output = _conv2d(output, kernel_size=1, stride=1, output_feature=out_feature,use_bias=False, name='pyramid_conv_1x1')
    ...
    def Squeeze_excitation_layer(self, input_x, out_dim, ratio, layer_name):
        with tf.name_scope(layer_name):
            ...
            excitation = self.PRelu(excitation)
    ...
    def cascade_feature(self,_input,block_num):
        ...
        out_feature = int(_input.get_shape()[-1])
        output = _input
        #output_1x1 = _conv3d(_input, kernel_size=1, stride=1, output_feature=out_feature, use_bias=False, name='pyramid_conv_1')

        for i in range(1, self.atrou_num + 1):
            #dilate_rate = int(np.power(2,4-block_num)*i)
            dilate_rate = int(np.power(2,4-block_num))
            #print(_input,dilate_rate)
            # dilate rate : 24 16 8 / 12 8 4 / 6 4 2
            output = atrous_bn_prelu(output, kernel_size=3, stride=1, output_channels=out_feature/2,
                             dilation_rate=dilate_rate, is_training=self.is_training,name='atrous_conv%d' % i)

            atrous_layer.append(output)

        output = tf.concat([_input, atrous_layer[0], atrous_layer[1], atrous_layer[2]], axis=-1)
    ...
    def _contracting_block(self, block_num, _input, input_layer):
        dilate_rate = int(2*(4 - block_num))
        # xception contracte blcok
        #output = atrous_bn_prelu(_input, kernel_size=3, stride=1, output_channels=self.output_channels[str(block_num)][0],
                         #name='conv_1', is_training=self.is_training,dilation_rate=dilate_rate)
        output = self.cascade_feature(_input,block_num)
        #output = atrous_bn_prelu(output, kernel_size=3, stride=1, output_channels=self.output_channels[str(block_num)][1],
                               #name='conv_2', is_training=self.is_training,dilation_rate=dilate_rate)
        ...
        #output = tf.layers.max_pooling2d(inputs=output, pool_size=2, strides=2, name='pool1')
        ...
        #input = _conv2d(_input, kernel_size=1, stride=2, output_feature=self.output_channels[str(block_num)][2], use_bias=False, name='conv_s2')
        ...
        #print(input)
        #output = self.Squeeze_excitation_layer(output, int(output.get_shape()[-1]), 16, layer_name='SE%d' % block_num)
    ...
    def inference_op(self, _input):
        ...
        with tf.device(device_name_or_function=self.device[0]):
            output = conv_bn_prelu(_input, output_channels=16, kernel_size=3, stride=1,
        ...
        with tf.device(device_name_or_function=self.device[1]):
            ...
            auxiliary3_prob_4x = _conv2d(inputs=dconv_layer[0], output_feature=self.output_classes, kernel_size=1,
                                         stride=1, use_bias=False, name='auxiliary3_prob_4x')
            ...
            auxiliary2_prob_2x = _conv2d(inputs=dconv_layer[1], output_feature=self.output_classes, kernel_size=1,
                                         stride=1, use_bias=False, name='auxiliary2_prob_2x')
            ...
            auxiliary1_prob_1x = _conv2d(inputs=dconv_layer[2], output_feature=self.output_classes, kernel_size=1,
                                         stride=1, use_bias=False, name='auxiliary1_prob_1x')

            with tf.variable_scope('last_stage'):
                # out_feature = int(output.get_shape()[-1]) / 2
                #print(dconv_layer[0],dconv_layer[1],dconv_layer[2])
                output1 = _conv2d(dconv_layer[0], kernel_size=1, stride=1, output_feature=5, use_bias=False,
                                  name='block1_conv1x1')
                ...
                output2 = _conv2d(dconv_layer[1], kernel_size=1, stride=1, output_feature=5, use_bias=False,
                                   name='block2_conv1x1')
                ...
                output3 = _conv2d(dconv_layer[2], kernel_size=1, stride=1, output_feature=5, use_bias=False,
                                   name='block3_conv1x1')

                output3 = tf.add(output2, output3)

                output = _conv2d(output3, kernel_size=1, stride=1, output_feature=self.output_classes, use_bias=False, name='fc_layer')
    ...
    def loss_op(self, outputs, labels, dst_weight=None):
        ...
        tf.summary.scalar("main_jacc_loss", self.main_dice_loss)
        tf.summary.scalar("total_jacc_loss", self.total_dice_loss)
