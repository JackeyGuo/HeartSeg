...
''' Fundamental 2D Convolution Definition '''


# 2D convolution
def conv2d(inputs, output_channels, kernel_size, stride, padding='same', use_bias=False, name='conv'):
    tensor = tf.layers.conv2d(
        ...
        data_format='channels_last',    # channels_last (batch, height, width, channels)
                                        # channels_first (batch, channels, depth, height, width)
        dilation_rate=(1, 1),        # incompatible problem with stride value != 1
...
def conv_bn_relu(inputs, output_channels, kernel_size, stride, is_training, name,
    ...
    with tf.variable_scope(name_or_scope=name):
        conv = conv2d(inputs, output_channels, kernel_size, stride, padding=padding,
...
def deconv2d(inputs, output_channels, name='deconv'):
    ...
    dev_filter = tf.get_variable(
        ...
        shape=[4, 4, output_channels, in_channels],
...
# 2D Deconvolution, Batch normalization, ReLU unit
