...
from core.deformable_convolution import DCN
...
def _conv2d(inputs, output_feature, kernel_size, stride, dilation_rate=1, padding='same', use_bias=False, name='conv'):
...
def _batch_norm(_input,is_training):
    ...
    output = tf.contrib.layers.batch_norm(
        _input, 0.99, scale=True,is_training=is_training,updates_collections=None)
    return output

def conv2d(_input, kernel_size, stride, output_feature, padding="SAME"):
...
def _conv2d_atrous(_input, kernel_size=3, stride=1, atrous=1, output_feature=8, padding="SAME"):
    in_features = int(_input.get_shape()[-1])
    kernel = _weight_variable_msra(
        [kernel_size, kernel_size, in_features, output_feature],
        name='kernel')
...
def _create_conv_layer(_input, kernel_size=3, stride=1, atrous=1, output_feature=16, padding="SAME", is_training=True):
    _input = _batch_norm(_input,is_training)
...
def _create_dconv_layer(_input, kernel_size=3, stride=1):
    ...
    output = tf.nn.conv2d_transpose(_input, kernel, output_shape, strides, padding='SAME', data_format='NDHWC')

    return output

def _deconv2d(inputs, input_shape, output_shape, stride=1, name='deconv'):
...
# 2D Deconvolution
def deconv2d(inputs, output_channels, name='deconv'):
    # depth, height and width
    batch, in_height, in_width, in_channels = [int(d) for d in inputs.get_shape()]
    dev_filter = tf.get_variable(
        name=name+'/filter',          # name of the new or existing variable
        shape=[3, 3, output_channels, in_channels],
    ...
    deconv = tf.nn.conv2d_transpose(
        value=inputs,                   # [batch, height, width, in_channels]
        filter=dev_filter,              # [height, width, output_channels, in_channels]
        output_shape=[batch, in_height*2, in_width*2, output_channels],
        strides=[1, 2, 2, 1],
        padding='SAME',
        data_format='NHWC',
        name=name
...
def _conv_layer(_input, kernel_size=3, stride=1, atrous=1, output_feature=16, padding="SAME"):
    ...
    if atrous == 1:
        output = _conv2d(_input, kernel_size, stride, output_feature, padding)
...
def conv_bn_prelu(inputs, output_channels, kernel_size, stride, is_training, name,
    ...
    with tf.variable_scope(name_or_scope=name):
        conv = _conv2d(inputs, output_channels, kernel_size, stride, padding=padding,
...
def bn_prelu_conv(inputs, output_channels, kernel_size, stride, is_training, name,
    ...
    with tf.variable_scope(name_or_scope=name):
        ...
        prelu = tflearn.prelu(bn, name=name+'_prelu')
        conv = _conv2d(prelu, output_channels, kernel_size, stride, padding=padding,
                       use_bias=use_bias, name=name + '_conv')
    return conv
...
def atrous_bn_prelu(inputs, output_channels, kernel_size, stride, is_training, name, dilation_rate,
                 padding='same', use_bias=False):
    with tf.variable_scope(name_or_scope=name):
        conv = _conv2d(inputs, output_channels, kernel_size, stride, padding=padding, dilation_rate=dilation_rate,
                      use_bias=use_bias, name=name+'_conv')
        '''device control?'''
        bn = tf.contrib.layers.batch_norm(
            inputs=conv,                # tensor, first dimension of batch_size
...
def deconv_bn_prelu(inputs, output_channels, is_training, name):
    with tf.variable_scope(name):
        deconv = deconv2d(inputs=inputs, output_channels=output_channels, name=name+'_deconv')
        '''device control?'''
...
def bn_prelu_deconv(inputs, output_channels, is_training, name):
    with tf.variable_scope(name):
        ...
        bn = tf.contrib.layers.batch_norm(inputs=inputs, decay=0.9, scale=True, epsilon=1e-5,
                                          ...
                                          scope=name+'_batch_norm')
        prelu = tflearn.prelu(bn, name=name+'_prelu')
        deconv = deconv2d(inputs=prelu, output_channels=output_channels, name=name + '_deconv')
    return deconv
...
def deform_conv2d(inputs, output_channels, kernel_size, name, norm=True, d_format='NHWC'):
    num_outputs = kernel_size[0] * kernel_size[1] * 2

    # 生成offset-field
    offset = tf.contrib.layers.conv2d(
        inputs, num_outputs, [3, 3], scope=name + '/offset',
...
def BilinearUpsample2d(inpt, up_factor):
    #inpt = tf.squeeze(inpt)

    #inpt_height, inpt_width, inpt_depth, n_inpt = inpt.get_shape().to_list()
    batch_size, inpt_height, inpt_width, n_inpt = [int(d) for d in inpt.get_shape()]
    ...
    #res = tf.image.resize_images(shuffle_res, [n_output])
    #print(res)
...
def convolution_2d(layer_input, filter, strides, padding='SAME'):
    ...
    w = tf.Variable(initial_value=xavier_uniform_dist_conv2d(shape=filter), name='weights')
    b = tf.Variable(tf.constant(1.0, shape=[filter[-1]]), name='biases')

    return tf.nn.conv2d(layer_input, w, strides, padding) + b

def v_conv2d(_input, kernel_shape, stride, padding="SAME"):
    #in_features = int(_input.get_shape()[-1])
...
def max_pooling_2d(layer_input, ksize, strides, padding='SAME'):
    assert len(ksize) == 4  # [batch, depth, rows, cols, channels]
    assert len(strides) == 4  # [batch, depth, rows, cols, channels]
    assert ksize[0] == ksize[3]
    assert ksize[0] == 1
    assert strides[0] == strides[3]
    assert strides[0] == 1
    return tf.nn.max_pool(layer_input, ksize, strides, padding)